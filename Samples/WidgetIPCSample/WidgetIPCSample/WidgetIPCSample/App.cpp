...
using namespace WidgetIPCSample;
using namespace WidgetIPCSample::implementation;
...
void App::OnActivated(IActivatedEventArgs const &e) {
  ...
  if (widgetArgs) {
    ...
    //
    // If IsLaunchActivation is true, this is Game Bar's initial activation of
    // us and we MUST create and hold onto XboxGameBarWidget.
    //
    ...
    if (widgetArgs.IsLaunchActivation()) {
      ...
      if (0 == appExtId.compare(L"WidgetIPC")) {
        m_widget1 = XboxGameBarWidget(
            widgetArgs, Window::Current().CoreWindow(), rootFrame);
  ...
  if (rootFrame == nullptr) {
    ...
    if (e.PrelaunchActivated() == false) {
      if (rootFrame.Content() == nullptr) {
        ...
        rootFrame.Navigate(xaml_typename<WidgetIPCSample::MainPage>(),
